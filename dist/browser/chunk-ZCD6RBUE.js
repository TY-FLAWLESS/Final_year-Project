...
function openDB(name4, version4, {
  blocked,
  upgrade,
  blocking,
  terminated
...
var environment = {
  ...
  apiUrl: "/api",
  firebaseConfig: {
    apiKey: "AIzaSyBRMe8X6pqI3qWrOEMaloK_7CEXTJFRP_4",
    authDomain: "finalyear-7df86.firebaseapp.com",
    projectId: "finalyear-7df86",
    storageBucket: "finalyear-7df86.firebasestorage.app",
    messagingSenderId: "101770480190",
    appId: "1:101770480190:web:5a80847bf763f01fab9f6f"
  }
};
...
var AuthService = class _AuthService {
  ...
  logout() {
    return from(signOut(this.firebaseAuth)).pipe(map(() => {
  ...
  updateUserName(email, name4) {
    const users = this.getUsers();
    const index = users.findIndex((item) => item.email.toLowerCase() === email.toLowerCase());
    if (index >= 0) {
      users[index].name = name4;
      this.saveUsers(users);
      const currentUser = this.getCurrentUser();
      if (currentUser?.email.toLowerCase() === email.toLowerCase()) {
        currentUser.name = name4;
        localStorage.setItem("unimart_user", JSON.stringify(currentUser));
      }
    }
  }
  ...
  login(payload) {
    return from(signInWithEmailAndPassword(this.firebaseAuth, payload.email, payload.password)).pipe(map((response) => {
  ...
  register(payload) {
    ...
    return from(createUserWithEmailAndPassword(this.firebaseAuth, payload.email, payload.password)).pipe(map((response) => {
      const user = {
        name: payload.name,
        email: response.user.email || payload.email,
        ...
        isActive: payload.role === "vendor" ? false : true,
      ...
      this.saveUsers(users);
      return {
        message: payload.role === "vendor" ? "Vendor registration successful! Awaiting admin approval." : "Registration successful! You can now log in.",
        user
      };
    }), catchError((error) => {
...
@firebase/auth/dist/esm2017/index-68602d24.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-ZCD6RBUE.js.map
